...
class ConsignmentAllocationDetail extends StatefulWidget {
  var FBillNo;

  ConsignmentAllocationDetail({Key? key, @required this.FBillNo}) : super(key: key);
...
class _RetrievalDetailState extends State<ConsignmentAllocationDetail> {
  ...
  var typeList = ["直接调拨单","VMI直接调拨单"];
  List<dynamic> typeListObj = [["ZJDB01_SYS","直接调拨单"],["ZJDB05_SYS","VMI直接调拨单"]];
  ...
  getStockList() async {
    ...
    if(this.organizationsNumber2 != null){
      userMap['FilterString'] = "FForbidStatus = 'A' and FUseOrgId.FNumber='" + this.organizationsNumber2 + "' and FStockProperty = 4";
    }else{
  ...
  getInventoryList() async{
    ...
    if(this.keyWord != '' && this.organizationsNumber1 != null){
      userMap['FilterString'] =
          "FMaterialId.FNumber='"+this.keyWord+"' and FBaseQty >0 and FStockOrgId.FNumber='" + this.organizationsNumber1 + "'";
    }else{
      if(this.keyWord == ""){
  ...
  getOrderList() async {
    ...
    userMap['FieldKeys'] =
    'FBillNo,FSaleOrgId.FNumber,FSaleOrgId.FName,FDate,FEntity_FEntryId,FMATERIALID.FNumber,FMATERIALID.FName,FMATERIALID.FSpecification,FOwnerTypeIdHead,FOwnerTypeIdHead,FUNITID.FNumber,FUNITID.FName,FRemainOutQty,FAPPROVEDATE,FNote,FID,FBACKUPSTOCKID.FNumber,FStockId.FNumber,FBillTypeID.FNUMBER,FEntity_FSeq,FMaterialId.FIsKFPeriod,FMaterialId.FExpPeriod,FMaterialId.FIsBatchManage,FLot.FNumber,FProduceDate,FExpiryDate,FBACKUPSTOCKID.FIsOpenLocation,FStockID.FIsOpenLocation,FDeliveryOrgID.FNumber,FDeliveryOrgID.FName,FDeliveryOrgID.FNumber,FDeliveryOrgID.FName,FBACKUPSTOCKID.FName,FStockId.FName,FBACKUPSTOCKLOCID.FF100002.FNumber,FStockLocID.FF100002.FNumber,F_UYEP_Date,FCustomerID.FNumber,FTaxRate,FTaxPrice,FAllAmount,FOrderNo,F_UYEP_Text,F_UYEP_UserId';
    Map<String, dynamic> dataMap = Map();
    dataMap['data'] = userMap;
    ...
    if (orderDate.length > 0) {
      ...
      for(var value in orderDate){
        ...
        arr.add({
          ...
          "value": { "label": value[23]==null?"":value[23], "value": value[23]==null?"":value[23]}
        });
        arr.add({
          "title": "调出仓库",
          "name": "FStockId",
        ...
        arr.add({
          ...
          "name": "FStockLocID",
          "isHide": false,
          "value": {"label": value[34]==null|| value[34] ==''?'':value[34], "value": value[34]==null|| value[34] ==''?'':value[34], "hide": value[26]}
        });
        arr.add({
          "title": "调入仓库",
          "name": "FStockId",
        ...
        arr.add({
          ...
          "name": "FStockLocID",
          "isHide": false,
          "value": {"label": value[35]==null|| value[35] ==''?'':value[35], "value": value[35]==null|| value[35] ==''?'':value[35], "hide": value[27]}
        });
        arr.add({
          "title": "最后扫描数量",
          "name": "FLastQty",
        ...
        if (barcodeData.length > 0) {
          ...
          if (msg == "") {
            ...
            this.getMaterialList(
                ...
                barcodeData[0][10],
                barcodeData[0][11],
                barcodeData[0][12].trim()==""?"":barcodeData[0][12].substring(0, 10),
                barcodeData[0][13].trim()==""?"":barcodeData[0][13].substring(0, 10),
                barcodeData[0][14],
                barcodeData[0][7]);
  ...
  saveOrder() async {
    if (this.hobby.length > 0) {
      ...
      Model['FID'] = 0;
      Model['FBillTypeID'] = {"FNUMBER": "ZJDB02_SYS"};
      Model['FDate'] = FDate;
      //获取登录信息
      SharedPreferences sharedPreferences =
      await SharedPreferences.getInstance();
      ...
      for (var element in this.hobby) {
        if (element[3]['value']['value'] != '0' && element[3]['value']['value'] != '' && element[3]['value']['value'] != 0 && element[8]['value']['value'] != '') {
          ...
          FEntityItem['FAllAmount'] = orderDate[hobbyIndex][40];
          FEntityItem['FTaxPrice'] = orderDate[hobbyIndex][39];
          FEntityItem['FTaxRate'] = orderDate[hobbyIndex][38];
