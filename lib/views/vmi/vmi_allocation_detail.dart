...
class VmiAllocationDetail extends StatefulWidget {
  var FBillNo;

  VmiAllocationDetail({Key? key, @required this.FBillNo}) : super(key: key);

  @override
  _RetrievalDetailState createState() => _RetrievalDetailState(FBillNo);
}

class _RetrievalDetailState extends State<VmiAllocationDetail> {
  ...
  getStockList() async {
    ...
    if(this.organizationsNumber2 != null){
      userMap['FilterString'] = "FForbidStatus = 'A' and FUseOrgId.FNumber='" + this.organizationsNumber2 + "'";
    }else{
      ...
      onConfirm: (p) {
        ...
        setState(() {
          if (hobby == 'organizations1') {
            ...
            data.forEach((element) {
              if (element == p) {
                typeNumber = typeListObj[elementIndex][0];
              }
              elementIndex++;
            });
  ...
  saveOrder() async {
    if (this.hobby.length > 0) {
      ...
      Model['FID'] = 0;
      Model['FBillTypeID'] = {"FNUMBER": this.typeNumber};
      Model['FDate'] = FDate;
      //获取登录信息
      SharedPreferences sharedPreferences =
      await SharedPreferences.getInstance();
            ...
            children: <Widget>[
              Expanded(
                child: ListView(children: <Widget>[
                  ...
                  _item('调拨类型',  this.typeList, this.typeName,
                      'type'),
